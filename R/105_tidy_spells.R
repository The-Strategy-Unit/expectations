...
# 1. TAKE THE FIRST SPELL FOR NTHROMBs, AND SPELL WITH THE THROMB 
#    (AND EXCL. SPELLS WITH TWO THROMBS) FOR THROMB PATIENTS; OR
# 2. TAKE THE SPELL IN THE SUPERSPELL FOR ALL 
# (HAVING FLAGGED ALL EPIS IN THROMB SPELL)

# FAVOUR 2., FOR CONSISTENCY AND CLEANNESS, 
# ESPECIALLY SINCE 94% THROMBECTOMIES HERE OCCUR IN FIRST EPI


# TRANSFERS (TO AND FROM A THROMB CENTRE) MUST BE DERIVED.
...
# df_spells <- df_thromb_exclusions |>
...
# SUPERSPELL FIRST SPELL ----------------------
# TAKE FIRST SPELL IN SUPER-SPELLS (WHETHER THE SUPER-SPELL HAS THROMB OR NOT)------

# SPLIT INTO THROMB AND NTHROMB:
df_thrombos <- df_thromb_exclusions |> 
  filter(thromb_spell == 1) |> 
  select(nhs_no, der_spell_id, admission_date, discharge_date)

df_nonthrombos <- df_thromb_exclusions |>
  filter(thromb_spell == 0) |> 
  select(nhs_no, der_spell_id, admission_date, discharge_date)

## 1. NON-THROMBECTOMY SPELLS THAT APPEAR TO BE SECONDARY SPELLS IN A SUPER SPELL ----
# REMOVES ~9Ok SPELLS
...
lkp_transfers_post_thromb <- df_nonthrombos |> 
  # TRANSFERS FROM SPECIALIST CENTRE: (REMOVES ~18K)
...
df_thromb_exclusions |> 
  filter(der_spell_id == "SUSPbR_1487162230602953391") |> 
  # filter(nhs_no == "100000000005" & der_spell_id == "SUSPbR_4482973574") |> 
  view("copy?")


df_flag_support_spells <- df_thromb_exclusions |> 
